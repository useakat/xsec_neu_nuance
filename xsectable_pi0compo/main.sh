...
if [[ "$1" == "-h" ]]; then
    echo ""
    echo "Usage: run.sh [mode] [nu] [years] [nrows] [mail]"
    echo ""
...
#	./submit_job.sh $job_system $que $i $jobname "../gen_xsec_nc_h2o_2.sh ${nucode} $E $nevent $output ${mode}"
	./submit_job.sh $job_system $que $i $jobname "../gen_xsec_nc_h2o_compo.sh ${nucode} $E $nevent $output ${mode}"
    fi
    i=`expr $i + 1`
done
n=$i
...
cp -rf par_1/nuance_defaults.cards output/.
